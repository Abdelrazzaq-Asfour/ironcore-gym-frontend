...
// ============================================================================
// Subscription Services (Mock vs Real) - متوافق مع SubscriptionController
// ============================================================================
export const subscriptionService = {
  async getActiveSubscription(userId) {
    if (USE_MOCK_DATA) {
      return mockDatabase.subscriptions.find(s => s.userId === Number(userId) && s.status === 'ACTIVE') || null;
    }

    try {
      const result = await apiClient.get(`http://localhost:8080/api/v1/subscriptions/user/${userId}`);
      return result.data || result;
    } catch (error) {
      return null; // إرجاع null بصمت في حال لم يوجد اشتراك نشط لتجنب أخطاء الواجهة
    }
  },

  async createOrUpdateSubscription(subscriptionData) {
    if (USE_MOCK_DATA) {
      // إلغاء تفعيل أي اشتراك سابق نشط لهذا المستخدم في الـ Mock
      mockDatabase.subscriptions.forEach(s => {
        if (s.userId === Number(subscriptionData.userId) && s.status === 'ACTIVE') {
          s.status = 'EXPIRED';
        }
      });
      ...
      mockDatabase.subscriptions.push(newSub);
      return newSub;
    }

    const result = await apiClient.post('http://localhost:8080/api/v1/subscriptions', subscriptionData);
    return result.data || result;
  }
};