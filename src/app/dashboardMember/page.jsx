...
export default function DashboardMemberPage() {
  ...
  const [bookingForm, setBookingForm] = useState({
    ...
    scheduleTime: '2026-08-30 10:00:00'
  });
...
try {
          // استخدام خدمة الاشتراكات المركزية مع معالجة الأخطاء بصمت
          const activeSub = await subscriptionService.getActiveSubscription(userId);
          if (activeSub && (activeSub.planTitle || activeSub.plan_title)) {
            setMyMembership({
              title: activeSub.planTitle || activeSub.plan_title,
              price_JOD: activeSub.priceJod || activeSub.price_jod,
              duration_months: activeSub.durationMonths || activeSub.duration_months,
              description: 'Active gym membership retrieved from database.'
            });
          }
        } catch (e) {
          console.warn("Subscription fetch handled safely, using fallback state.");
        }
  ...
  const handleCreateBooking = async (e) => {
    ...
    try {
      const payload = {
        ...
        className: bookingForm.className,
        trainerName: bookingForm.trainerName,
        scheduleTime: bookingForm.scheduleTime,
        status: 'CONFIRMED'
      };

      const newBooking = await bookingService.createBooking(payload);
      setMyBookings((prev) => [...prev, newBooking]);
      setMessage(`Successfully booked "${bookingForm.className}" with ${bookingForm.trainerName}!`);
    } catch (err) {
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between">
      {/* Top Navbar */}
      <nav className="border-b border-slate-800/80 bg-slate-900/50 backdrop-blur-md px-8 py-4 flex justify-between items-center sticky top-0 z-50">
      ...
      <div className="flex-1 max-w-7xl w-full mx-auto px-6 py-10 space-y-10">
        ...
        {/* Book a New Training Session Section */}
        <div className="bg-slate-900/60 border border-slate-800/80 rounded-2xl p-6 shadow-xl">
          <h2 className="text-lg font-bold text-white mb-2">Book a New Training Session</h2>
          <p className="text-slate-400 text-sm mb-6">Select your preferred class and expert trainer to lock in your schedule.</p>
          ...
          <form onSubmit={handleCreateBooking} className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <select
                ...
                className="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 text-slate-100 text-sm focus:outline-none focus:border-red-600"
              >
                <option value="Weightlifting & Power Classes">Weightlifting & Power Classes</option>
            ...
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Preferred Trainer</label>
              <select
                value={bookingForm.trainerName}
                onChange={(e) => setBookingForm({ ...bookingForm, trainerName: e.target.value })}
                className="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 text-slate-100 text-sm focus:outline-none focus:border-red-600"
              >
                <option value="coach_mahmoud">Coach Mahmoud</option>
                <option value="coach_rami">Coach Rami</option>
                <option value="coach_sara">Coach Sara</option>
              </select>
            </div>

            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Schedule Time & Date</label>
              <input
                type="datetime-local"
                value={bookingForm.scheduleTime.replace(' ', 'T')}
                onChange={(e) => setBookingForm({ ...bookingForm, scheduleTime: e.target.value.replace('T', ' ') + ':00' })}
                className="w-full px-4 py-3 rounded-xl bg-slate-950 border border-slate-800 text-slate-100 text-sm focus:outline-none focus:border-red-600"
              />
            </div>
            ...
            <div className="md:col-span-3 mt-2">
              <button
                ...
                className="bg-red-600 hover:bg-red-700 text-white font-bold text-xs uppercase tracking-wider px-6 py-3.5 rounded-xl shadow-lg shadow-red-600/20 transition-all disabled:opacity-50"
              >
                {bookingLoading ? 'Processing Booking...' : 'Confirm Session Booking'}
              </button>
            </div>
          </form>
        </div>
              ...
              <table className="w-full text-left text-sm text-slate-300">
                ...
                <tbody className="divide-y divide-slate-800/60">
                  {myBookings.map((booking) => (
                    <tr key={booking.id || Math.random()} className="hover:bg-slate-900/40">
